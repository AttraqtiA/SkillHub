            ...
            <form action="{{ route('course_classes.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                              required
                              class="w-full rounded-md border border-slate-300 bg-slate-50 shadow-sm focus:bg-white focus:border-skillhub-500 focus:ring-skillhub-500 text-sm">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        {{-- status field removed from migration, so no input here --}}
                        <div class="text-xs text-slate-500 italic">
                            Status is handled implicitly (e.g. via dates) and not stored as a column.
                        </div>
                    </div>
                </div>
